import Image from "next/image";

export default function Gallery() {
  return (
    <section
      className="pt-8 md:pt-12 container mx-auto px-8 xl:px-24 pb-8 overflow-hidden rounded-md"
      data-aos="fade-up"
    >
      <h2 className="text-4xl text-center font-medium">Gallery</h2>
      <div
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pt-8 items-start"
        data-aos="fade-up"
        data-aos-delay="200"
      >
        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="AWS Deeprace Dash"
            src="/events/AWS Deeprace Dash/photo_2024-04-11_22-35-26.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">AWS Deeprace Dash</p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="AWS Deeprace Dash"
            src="/events/AWS Deeprace Dash/photo_2024-04-11_22-35-34.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">AWS Deeprace Dash</p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="AWS Deeprace Dash"
            src="/events/AWS Deeprace Dash/photo_2024-04-11_22-37-17.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">AWS Deeprace Dash</p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="Cedar Girls Secondary School"
            src="/events/Cedar Girls Secondary School/photo_2024-04-22_17-46-19.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">
              Cedar Girls Secondary School
            </p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="Cedar Girls Secondary School"
            src="/events/Cedar Girls Secondary School/photo_2024-04-22_17-46-24.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">
              Cedar Girls Secondary School
            </p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="MindfulHacks"
            src="/events/MindfulHacks x Singapore Youth AI/photo_2024-04-22_17-47-37.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">
              MindfulHacks x Singapore Youth AI
            </p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="MindfulHacks"
            src="/events/MindfulHacks x Singapore Youth AI/photo_2024-04-22_17-47-54.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">
              MindfulHacks x Singapore Youth AI
            </p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="Community Talks"
            src="/events/Singapore Youth AI inaugural community gathering/photo_2024-04-11_22-20-51.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">
              Singapore Youth AI inaugural community gathering
            </p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="Community Talks"
            src="/events/Singapore Youth AI inaugural community gathering/photo_2024-04-11_22-20-59.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">
              Singapore Youth AI inaugural community gathering
            </p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="Community Talks"
            src="/events/Singapore Youth AI inaugural community gathering/photo_2024-04-11_22-21-01.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">
              Singapore Youth AI inaugural community gathering
            </p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="YAC"
            src="/events/Youth Action Challenge/photo_2024-04-11_22-44-47.jpg"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">Youth Action Challenge</p>
          </div>
        </div>

        <div className="overflow-hidden bg-white rounded-lg shadow-md">
          <Image
            width={1000}
            height={1000}
            alt="YAC"
            src="/events/Youth Action Challenge/Screenshot 2024-04-11 224643.png"
            className="w-full h-auto"
          />
          <div className="p-4">
            <p className="text-center font-medium">Youth Action Challenge</p>
          </div>
        </div>
      </div>
    </section>
  );
}
